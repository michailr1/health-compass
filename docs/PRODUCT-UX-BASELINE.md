...
### Принятые решения по навигации и копирайту — 2026-07-13 (owner review)

По итогам просмотра production-навигации владельцем проекта приняты
следующие обязательные решения. Они фиксируют цель; реализация — отдельная
задача после текущего HC-017 slice и не должна смешиваться с его rollout.

1. **Мобильная навигация нарушает baseline и подлежит пересмотру.**
   В production нижняя панель содержит 7 пунктов (Дашборд, Документы, Oura,
   Генетика, План, Источники, История) при принятом правиле «не более пяти».
   Целевая схема остаётся прежней: Главная · История · Добавить · Ассистент ·
   Ещё. Demo-разделы (Генетика, План, Источники) и интеграции уходят из
   нижней панели в «Ещё»/настройки.
2. **Ярлык «Документы» не сообщает пользователю ценность.**
   Раздел должен называться в терминах пользовательской задачи
   (рабочий вариант — «Анализы»; финальный ярлык выбирается при реализации).
   Пустое состояние обязано отвечать: что загружать (PDF или фото
   результатов анализов), зачем (значения попадут в показатели, динамику и
   отчёты) и что будет дальше (распознанное остаётся черновиком до проверки
   человеком).
3. **Vendor-брендированные вкладки верхнего уровня запрещены.**
   В навигации живут домены данных (например, «Сон»), а не производители.
   Вкладка «Oura» заменяется доменом «Сон»; подключение конкретных устройств
   (Oura, Apple Health / Health Connect по отдельному ADR и т.д.) выполняется
   внутри «Источников»/настроек интеграций. Одна интеграция не может
   определять структуру навигации.
4. **CTA пустого дашборда «Подключить источник» скрывается**, пока нет ни
   одной реально работающей интеграции. Первичным CTA пустого состояния
   служит выполнимое действие: заполнить профиль здоровья или загрузить
   анализ (когда загрузка включена).
5. **Копирайт «Защищённый приём документов» очищается от
   developer-speak.** Фраза «Имя файла не используется как путь хранения, а
   исходник недоступен для просмотра, пока находится в карантине» описывает
   внутренние механизмы (path traversal, карантинный доступ) и пользователю
   не нужна. Технические гарантии остаются в `SECURITY-INVARIANTS.md` и
   HC-017 документации. Пользовательский текст говорит о ценности и
   ожиданиях, например: «Файл хранится в зашифрованном виде и виден только
   вам. После проверки файла мы распознаем текст — вы сможете просмотреть и
   подтвердить результат». Слово «карантин» в UI не используется;
   допустимо «идёт проверка файла».
