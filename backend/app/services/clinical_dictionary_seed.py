...
def load_seed_manifest(path: str | Path) -> SeedManifest:
    ...
    source_label = _require_non_empty_string(payload.get("source_label"), "source_label")
    ...
    for index, raw in enumerate(raw_concepts):
        ...
        domain = _require_non_empty_string(raw.get("domain"), f"concepts[{index}].domain")
        if domain not in ALLOWED_DOMAINS:
            raise ValueError(f"unsupported domain: {domain}")

        display_name = _require_non_empty_string(
            raw.get("display_name"),
            f"concepts[{index}].display_name",
        )
        locale = _require_non_empty_string(raw.get("locale"), f"concepts[{index}].locale")
        country = _require_non_empty_string(raw.get("country"), f"concepts[{index}].country")
        if locale != "ru-RU":
        ...
        for alias_index, raw_alias in enumerate(raw_aliases):
            ...
            alias_text = _require_non_empty_string(
                raw_alias.get("text"),
                f"concepts[{index}].aliases[{alias_index}].text",
            )
            ...
            aliases.append(
                SeedAlias(
                    ...
                    kind=_require_non_empty_string(
                        raw_alias.get("kind"),
                        f"concepts[{index}].aliases[{alias_index}].kind",
                    ),
                )
            )
        ...
        concepts.append(
            SeedConcept(
                ...
                source_system=_require_non_empty_string(
                    ...
                    f"concepts[{index}].source_system",
                ),
                source_code=_optional_string(
                    raw.get("source_code"),
                    f"concepts[{index}].source_code",
                ),
                qualifier=_optional_string(raw.get("qualifier"), f"concepts[{index}].qualifier"),
                locale=locale,
                country=country,
