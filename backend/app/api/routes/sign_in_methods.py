...
async def start_settings_link(
    ...
    if provider == "google":
        ...
        if source is None or (source.claims or {}).get("email_verified") is not True:
            raise HTTPException(
                ...
                detail="A verified Email Magic Link identity is required",
            )
        ...
        if source is None or source_claims.get("email_verified") is not True:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="A verified Google identity is required",
            )
        normalized_email = normalize_email(str(source_claims.get("email") or ""))
        if not normalized_email:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Google email is unavailable")
    ...
    intent_id = await create_account_link_intent(
        ...
        initiating_provider=source.provider,
        initiating_subject=source.subject,
        ...
        initiating_claims=source.claims or {},
